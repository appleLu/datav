...
func DeleteUser(c *gin.Context) {
	...
	if targetUser.Username == models.SuperAdminUsername {
		c.JSON(400,common.ResponseErrorMessage(nil,i18n.OFF,"username 'admin' mustn't be deleted"))
		return 
	}
	...
	if teamCount > 0 {
		c.JSON(400,common.ResponseErrorMessage(nil,i18n.OFF, fmt.Sprintf("this user has %d teams,please transfer or delete teams first",teamCount)))
		return 
	}
	...
	if !acl.IsUserSelf(user.Id,c) {
		...
		if oldUser.Username == models.SuperAdminUsername {
			if user.Username != models.SuperAdminUsername  {
				c.JSON(400,common.ResponseErrorMessage(nil,i18n.OFF,"You cannot change username 'admin' to other username"))
				return 
			}
	
			if !user.Role.IsAdmin() {
				c.JSON(400,common.ResponseErrorMessage(nil,i18n.OFF,"You cannot change username 'admin' to other role"))
				return 
			}
		}
	}
...
func UpdatePassword(c *gin.Context) {
	...
	if user.Username == models.SuperAdminUsername {
		if !acl.IsSuperAdmin(c) {
			c.JSON(400,common.ResponseErrorMessage(nil,i18n.OFF,"You cannot change the password of username 'admin'"))
			return 
		}
	}
